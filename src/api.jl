...
function pseudo_vio(gnep::AbstractGNEP{T, S}, x::S) where {T, S}
  ...
  for i = 1:(gnep.N)
    ...
    nI = (c + 1):(c + ncon)
    cx[nI] =
      max.(cons(gnep.player[i], x) - gnep.player[i].meta.ucon, 0) +
      max.(-cons(gnep.player[i], x) + gnep.player[i].meta.lcon, 0)
    c += ncon
  end
  return cx
end
...
function lagrange_multiplier(gnep::AbstractGNEP{T, S}, x::S; prec::Float64 = 1e-6) where {T, S}
  ...
  for i = 1:(gnep.N)
    ...
    ncP = findall(
      min.(abs.(cx - gnep.player[i].meta.ucon), abs.(gnep.player[i].meta.lcon - cx)) .<= prec,
    )
    ncB = findall(
      min.(
        abs.(x[nI] - gnep.player[i].meta.uvar[nI]),
        abs.(gnep.player[i].meta.lvar[nI] - x[nI]),
      ) .<= prec,
    )
    J = vcat(jac(gnep.player[i], x)[ncP, nI], Matrix(1.0I, nn, nn)[ncB, :])
    lambda[c .+ union(ncP, ncon .+ ncB)] =
      -LinearAlgebra.pinv(convert(Array, J')) * grad(gnep.player[i], x)[nI]
    c += ncon + nn
  end
